# For continuous outcome, prepare  genotype and phenotype data for true and misspecified  models in order to run SAIGE-GENE
library(gdsfmt)
library(SeqArray)
library(STAAR)
library(SKAT)
...
es1_values <- c(0.1, 0.2, 0.3, 0.4)
es2_values <- c(-0.1, -0.2, -0.3, -0.4)
...
SCENARIOS_PER_SET <- NUM_es1 * NUM_es2  # 4 * 4 = 16
...
output_path <- paste0("/rsrch6/home/biostatistics/nkui/saige/maf0.2C/")
...
pheno_<- function(beta0, es1, es2, Gc, causal_indices, runs, n, k, es_fixed, es_fixed_name) 
{
  # --- SCENARIO SETUP ---
  scenario_name <- paste0("es_fixed", es_fixed_name, "_es1_", es1, "_es2_", es2)
  cat(sprintf("\n--- Running Scenario: %s ---\n", scenario_name))
  ...
  output_path <- paste0("/rsrch6/home/biostatistics/nkui/saige/maf0.2C/","job",job_index)
